...
public interface BudgetAllocationQuery {
    ...
    String fields = allocation_id + "," + transaction_id + "," + category_id + "," + amount + "," + create_ts + "," + update_ts;

    @SqlQuery("insert into " + table + " (" +
            transaction_id + "," + category_id + "," + amount  +
            ") values (" +
            ":" + transaction_id + ", :" + category_id + ", :" + amount + ") returning " + fields)
    ...
    @Mapper(BudgetAllocationMapper.class)
    List<BudgetAllocation> findByTransactionId(@Bind("transaction_id") String transaction_id);

    @SqlQuery("select " + fields + " from " + table + " where :allocation_id = allocation_id")
    @Mapper(BudgetAllocationMapper.class)
    BudgetAllocation findById(@Bind("allocation_id") long allocation_id);
