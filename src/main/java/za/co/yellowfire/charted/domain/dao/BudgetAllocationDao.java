 ...
 * @version 0.1.0
 */
public class BudgetAllocationDao extends BaseDao<BudgetAllocation> implements DomainObjectDao<BudgetAllocation> {
    ...
    public BudgetAllocation findById(final Long id) {
        return dbi.withHandle(new HandleCallback<BudgetAllocation>() {
            @Override
            public BudgetAllocation withHandle(Handle handle) throws Exception {
                BudgetAllocationQuery query = handle.attach(BudgetAllocationQuery.class);
                TransactionQuery transactionQuery = handle.attach(TransactionQuery.class);
                BudgetCategoryQuery budgetCategoryQuery = handle.attach(BudgetCategoryQuery.class);

                return build(query.findById(id), transactionQuery, budgetCategoryQuery);
            }
        });
    }

    public List<BudgetAllocation> findByTransactionId(final String transactionId) throws DataException {
        return dbi.withHandle(new HandleCallback<List<BudgetAllocation>>() {
            @Override
            public List<BudgetAllocation> withHandle(Handle handle) throws Exception {
                BudgetAllocationQuery query = handle.attach(BudgetAllocationQuery.class);
                ...
                List<BudgetAllocation> results = build(query.findByTransactionId(transactionId), transactionQuery, budgetCategoryQuery);
                return (results != null && results.size() > 0) ? results : new ArrayList<BudgetAllocation>(0);
            }
        });
    }
